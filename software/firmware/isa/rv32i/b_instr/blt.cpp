...
uint32_t generate_blt(Arguments* args)
{
    uint8_t  rs1;
    uint8_t  rs2;
    uint32_t uimm;
    ...
    // taken blt to jump to begin
    fprintf(file,
    "   addi x1, x0, 1\n"
    "   blt x0, x1, begin  # Instruction under test.\n\n"
    );

    fprintf(file,
    "back:\n"
    "   addi x1, x0, 1\n"
    "   blt x0, x1, _end  # Instruction under test.\n\n"
    );

    // Set of not taken blt
    fprintf(file,
    "begin:\n"
    );
    ...
    for(uint8_t i = 0; i < args->nb_instr - 3; i++)
    {
        ...
        fprintf(file,
        "   blt x%hhu, x%hhu, _end  # Instruction under test.\n\n",
        rs1, rs2
        );
    }
    ...
    fprintf(file,
    "   blt x%hhu, x%hhu, back  # Instruction under test.\n\n",
    rs1, rs2
    );
...
int main(int argc, char** argv)
{
    ...
    if(generate_blt(&args) != 0x00) { exit(EXIT_FAILURE); }
