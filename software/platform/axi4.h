 ...
 *
 * \param[in] addr   Start byte address (AXI space relative to the instr
 * window). \param[in] data   Pointer to 32-bit words to write (source buffer).
 * Must be non-null if \p size > 0. \param[in] size   Number of bytes to write
 ...
 * target, ensure \ref SetupAxi4 was called beforehand. In simulation, the
 * backend drives the DUT AXI directly.
 *
 * \param[in] addr   Start byte address (AXI space relative to the mapped
 * window). \param[in] data   Pointer to words to write (source buffer). Must be
 * non-null if \p size > 0. \param[in] size   Number of bytes to write (must be
 * a multiple of NB_BYTES_IN_WORD).
 *
 * \retval SUCCESS            Transfer completed.
 * \retval ADDR_NOT_ALIGNED   \p addr or \p size not aligned to
 * NB_BYTES_IN_WORD. \retval INVALID_ADDR       AXI window not mapped on
 * platform target. \retval FAILURE            Invalid data pointer or size.
 */
 ...
 *
 * \param[in]  addr   Start byte address (AXI space relative to the mapped
 * window). \param[in]  size   Number of bytes to read (must be a multiple of
 * NB_BYTES_IN_WORD). \param[out] data   Destination buffer for read words. Must
 * be non-null if \p size > 0.
 *
 * \retval SUCCESS            Transfer completed (buffer filled).
 * \retval ADDR_NOT_ALIGNED   \p addr or \p size not aligned to
 * NB_BYTES_IN_WORD. \retval INVALID_ADDR       AXI window not mapped on
 * platform target. \retval FAILURE            Backend-specific failure.
 */
