...
#(
    ...
    input  wire [    DATA_WIDTH - 1 : 0] exe_bypass_i,
    /// Mem to Decode bypass
    input  wire [    DATA_WIDTH - 1 : 0] mem_bypass_i,
    /// Writeback to Decode bypass
    input  wire [    DATA_WIDTH - 1 : 0] wb_bypass_i,
  ...
  * Otherwise, it takes CSR_RD is read only or CSR_IDLE if no CSR operation.
  */
  always_comb begin : csr_ctrl_gen
    if (!rstn_i) begin
      ...
      if (op == SYS_OP) begin
        case (funct3)
          3'b000, 3'b100: csr_ctrl = CSR_RD;
          3'b001, 3'b101: csr_ctrl = CSR_ALU;
          default:        csr_ctrl = |instr_i[19:15] ? CSR_RD : CSR_IDLE;
        endcase
      end
      else begin
        csr_ctrl = CSR_IDLE;
  ...
  always_comb begin : operands_gen
      ...
      case (op)
        ...
        SYS_OP: begin
          if (funct3 == 0) begin
            ...
            op1 = (funct3 == 3'b111) ? ~{{DATA_WIDTH - 5{1'b0}}, instr_i[19:15]} :
                {{DATA_WIDTH - 5{1'b0}}, instr_i[19:15]};
          end
          else begin
    ...
    if (|op2_sel_i) begin
      ...
      case (op)
        ...
        LOAD_OP: op2 = {{DATA_WIDTH - 12{instr_i[31]}}, instr_i[31:20]};

        STORE_OP: op2 = {{DATA_WIDTH - 12{instr_i[31]}}, instr_i[31:25], instr_i[11:7]};
        ...
        JALR_OP: op2 = {{DATA_WIDTH - 12{instr_i[31]}}, instr_i[31:20]};
